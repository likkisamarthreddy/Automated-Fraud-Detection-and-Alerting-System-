...
import { AlertTriangle, MessageSquare, UserCheck, Clock, CheckCircle, Ban, Search } from "lucide-react";
...
const Alerts = () => {
  ...
  const handleAction = async (id: number, action: 'block' | 'allow' | 'review') => {
    try {
      if (action === 'block') await alertService.blockAlert(id);
      if (action === 'allow') await alertService.allowAlert(id);
      if (action === 'review') await alertService.reviewAlert(id);
      
      toast.success(`Alert transaction ${action}ed successfully`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: "Actioned", count: metrics?.resolvedAlerts || 0, icon: UserCheck, cls: "text-success" },
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${statusStyles[alert.status] || statusStyles.OPEN}`}>
                      {alert.status}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-4">
                    {alert.status === "OPEN" && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleAction(alert.alertId, 'block')}
                        ...
                        <button
                          onClick={() => handleAction(alert.alertId, 'allow')}
                        ...
                        <button
                          onClick={() => handleAction(alert.alertId, 'review')}
